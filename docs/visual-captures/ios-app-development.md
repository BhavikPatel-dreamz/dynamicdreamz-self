...
| Assets | Canonical brand logos in `public/assets/clients/`, 3 unique service SVGs in `public/assets/services/ios-app-development/` + 6 reused canonical SVGs, 4 canonical portfolio project screenshots in `public/assets/our-work/projects/` (BellaVita Organic, Renee Cosmetics, Rentastic, Journal X) with Apple App Store platform icon, and 11 testimonial photos. |

## Section Inventory

| Section | Live behavior/style | Local implementation notes |
| --- | --- | --- |
| Hero | `.inner-hero-sec`: left h1 `Custom iOS Mobile App Development Services` (50/66 → 40/50 → 30/40 → 34/44), paragraph, `REQUEST A QUOTE` red pill to `/request-quote`; right: 3 rotating 420px circles (Clutch #F8DDD7, Upwork #E3F1D5, GoodFirms #D8E5FF) with ratings & review counts; 275px circle ≤767px. | Reused `ServiceHeroSection` + client `ReviewAnimation` component with full timing parity; links to internal `/request-quote`. |
| Brands | `.our-client-sec` #faf4ee with heading `Trusted by <br>Leading Brands` + 12 brand logos (Supertails, Eleven Eleven, Bella Vita, Bombay Shirts, Popclub, Sri Sri Tattva, Tropicfeel, Renee, Royce Chocolate, Tego, Nekter, Rare Rabbit). | Reused `IndustryBrandsSection` with typed `iosAppDevelopmentBrands`. |
| Services ("What We Provide") | `.what-we-provide-sec` heading-text + 9 `.services-box` cards (Prototyping, Wireframes & Mockups; Native iOS App Development; UI/UX Design; Backend Development; App Testing and Quality Assurance; App Maintenance and Support; Existing App Customization; Shopify App Development; Hybrid iOS App Development) + centered `LET ME GIVE YOU A HAND TO HELP YOU` red pill to `/request-quote`. | Reused `AgencyServicesSection` with typed `iosAppDevelopmentServices`. |
| Portfolio ("Glimpses of Our IOS App Developments") | `.our-work-sec` 4-col grid (`.grid-column-4`) of `.our_work_team.mobile_apps` cards (BellaVita Organic, Renee Cosmetics, Rentastic, Journal X) with `IOS APP DEVELOPMENT` eyebrow, `padding-bottom: 154.5%`, dark hover overlay, Apple App Store links, and `View Project` link + `VIEW OUR WORK` CTA button to `/our-work`. | Reused `PortfolioShowcaseSection` with `columns={4}`, `imageAspectClassName="pb-[154.5%] max-[767px]:pb-[115%]"`, and typed `iosAppDevelopmentPortfolio`. |
...
| FAQs | `.faq-sec` white bg: h2 `Frequently Asked Questions` + 6 accordion items (first open), plus/minus icon, 16px/500 answers. | Reused `FaqSection` with `iosAppDevelopmentFaqs`. |
...
| Portfolio hover | 40% black overlay, View Project rises to 30px, Apple App Store platform mark fades in | Matches `PortfolioProjectCard` | verified |
...
| --- | --- | --- |
| Live canonical/og:url have trailing slash | Slashless `/ios-app-development` per project URL policy | implemented |
| Live title length | `iOS App Development Services | Dynamic Dreamz` (45 chars - within 60-char budget) | implemented |
| Live description length | Preserved live description (126 chars - within 70-160 char budget) | implemented |
