# Cross-Platform App Development Page
...
| Viewport | Status |
| --- | --- |
| 1440x900 (Desktop) | Verified layout structure, heading hierarchy, 2-column hero with 420px animated review wheel, 12-logo brand slider, 8 service cards ("What We Provide"), 4-column portfolio showcase ("Glimpses of Our Cross Platform App Development"), 11 video testimonials slider ("Don't Just Take Our Word For It"), 8 FAQ accordions, and bottom CTA banner. |
...
| Assets | Canonical brand logos in `public/assets/clients/`, 2 unique service SVGs in `public/assets/services/cross-platform-app-development/` + 6 reused canonical SVGs, 2 unique platform SVGs in `public/assets/our-work/platforms/` (`react-logo.svg`, `flutter-icon.svg`), 4 canonical portfolio project screenshots in `public/assets/our-work/projects/` (Bombay Shirt Company, Llama – An App by CWRB, Renee Cosmetics, Supertails), and 11 testimonial photos. |

## Section Inventory

| Section | Live behavior/style | Local implementation notes |
| --- | --- | --- |
| Hero | `.inner-hero-sec`: left h1 `Custom Cross Platform App Development Services` (50/66 → 40/50 → 30/40 → 34/44), paragraph, `Request a Quote` red pill to `/request-quote`; right: 3 rotating 420px circles (Clutch #F8DDD7, Upwork #E3F1D5, GoodFirms #D8E5FF) with ratings & review counts; 275px circle ≤767px. | Reused `ServiceHeroSection` + client `ReviewAnimation` component with full timing parity; links to internal `/request-quote`. |
| Brands | `.our-client-sec` #faf4ee with heading `Trusted by <br>Leading Brands` + 12 brand logos (Supertails, Eleven Eleven, Bella Vita, Bombay Shirts, Popclub, Sri Sri Tattva, Tropicfeel, Renee, Royce Chocolate, Tego, Nekter, Rare Rabbit). | Reused `IndustryBrandsSection` with typed `crossPlatformAppDevelopmentBrands`. |
| Services ("What We Provide") | `.what-we-provide-sec` heading-text + 8 `.services-box` cards (Flutter App Development; React Native App Development; Prototyping, Wireframes & Mockups; Backend Development; App Testing and Quality Assurance; App Maintenance and Support; Existing App Customization; Shopify App Development) + centered `Let me give you a hand to help you` red pill to `/request-quote`. | Reused `AgencyServicesSection` with typed `crossPlatformAppDevelopmentServices`. |
| Portfolio ("Glimpses of Our Cross Platform App Development") | `.our-work-sec` 4-col grid (`.grid-column-4`) of `.our_work_team.mobile_apps` cards (Bombay Shirt Company, Llama – An App by CWRB, Renee Cosmetics, Supertails) with `React Native` and `Flutter` category eyebrows, `padding-bottom: 154.5%`, dark hover overlay, platform badges, Google Play & Apple App Store links, and `View Project` link + `View our work` CTA button to `/our-work`. | Reused `PortfolioShowcaseSection` with `columns={4}`, `imageAspectClassName="pb-[154.5%] max-[767px]:pb-[115%]"`, and typed `crossPlatformAppDevelopmentPortfolio`. |
...
| FAQs | `.faq-sec` white bg: h2 `Frequently Asked Questions` + 8 accordion items (first open), plus/minus icon, 16px/500 answers. | Reused `FaqSection` with `crossPlatformAppDevelopmentFaqs`. |
...
| Portfolio hover | 40% black overlay, View Project rises to 30px, React/Flutter platform mark fades in | Matches `PortfolioProjectCard` | verified |
...
| --- | --- | --- |
| Live canonical/og:url have trailing slash | Slashless `/cross-platform-app-development` per project URL policy | implemented |
| Live title length | `Cross-Platform App Development Services | Dynamic Dreamz` (56 chars - within 60-char budget) | implemented |
| Live description length | `We build robust, high-performance apps that work seamlessly across iOS and Android. Partner with us to deliver consistent cross-platform user experiences.` (153 chars - within 70-160 char budget) | implemented |
