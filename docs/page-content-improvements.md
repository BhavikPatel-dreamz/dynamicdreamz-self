...
| CTA and service destinations | `/request-quote` and `/bigcommerce-development` are not yet built. The local consultation CTA now uses the implemented `/book-a-discovery-call` route. | Preserve the remaining canonical path intent during migration and validate, implement, or redirect it before launch. | Restores the consultation conversion flow while retaining the active-migration exception for the remaining destinations. | High | partially implemented; consultation CTA remapped 2026-09-07 |
...
| Area | Current UI Copy | Suggested UI Copy | Reason | Priority | Status |
| --- | --- | --- | --- | --- | --- |
| Meta title | `Shopify Development Agency & Services \| Dynamic Dreamz` (55 chars) | `Shopify Development Agency & Services \| Dynamic Dreamz` (55 chars) | Preserves live title; fits SEO 60-character budget and avoids truncation in search results while targeting primary search intent. | High | implemented in metadata |
| Meta description | `Looking for a Shopify development agency? Work with a Shopify Platinum Partner for custom builds, migrations, integrations, CRO and ongoing support.` (154 chars) | `Looking for a Shopify development agency? Work with a Shopify Platinum Partner for custom builds, migrations, integrations, CRO and ongoing support.` (154 chars) | Preserves live Yoast SEO description; fits 70-160 character budget and accurately reflects full service capabilities. | High | implemented in metadata |
| Hero subtitle removal | Phantom `subtitle` element was present in local content | Removed subtitle so H1 is directly followed by lead paragraph matching live DOM structure. | Aligns with live DOM hierarchy. | High | implemented |
| Brand logo set | Outdated 10 brand logos | Updated to 12 exact live client brands (Tego, Nekter, Rare Rabbit, Supertails, Eleven Eleven, Bella Vita, Bombay Shirt Company, Popclub, SriSri Tattva, Tropicfeel, Renee, Royce). | Replaces outdated client list with 1:1 live parity. | High | implemented |
...
Status: visible recommendations deferred; live UI preserved during migration
Last reviewed: 2026-09-24
Primary SEO intent: Shopify Plus migration agency, Shopify Plus migration, migrate to Shopify Plus, Shopify migration experts, Shopify Plus migration services.

| Area | Current UI Copy | Suggested UI Copy | Reason | Priority | Status |
| --- | --- | --- | --- | --- | --- |
| Meta title | `Shopify Plus Migration Agency \| Dynamic Dreamz` (48 chars) | Preserved live title (48 chars). | Complies with repository 15-60 character limit and matches latest live title. | High | implemented in metadata |
| Meta description | `Migrate to Shopify Plus with a Shopify Platinum Partner. We handle data, design, apps, integrations, SEO redirects, QA, launch and ongoing support.` (147 chars) | Preserved live description (147 chars). | Complies with repository 70-160 character limit and matches latest live description. | High | implemented in metadata |
| Hero eyebrow casing | `Established in 2006` / `Shopify Plus Migration Agency` | Keep live eyebrow text and formatting. | Preserves brand authority and search keyword alignment without altering rendered appearance. | Low | implemented in content |
| Process steps casing | `Audit & discovery`, `Plan & architecture`, `Design & development`, etc. | Sentence case matching live site | Preserves live-site typography exactly. | Low | implemented in content |
...
| Area | Current UI Copy | Suggested UI Copy | Reason | Priority | Status |
| FAQ 1-7 Location Name | Questions 1 to 7 literally ask about "Texas" on the live Miami URL (e.g., "What makes Dynamic Dreamz the best Shopify development company in Texas?", "How much does Shopify development cost in Texas?", etc.) | Update questions and answers to reference "Miami" instead of "Texas" once explicitly approved by project owner | Fixes geographic mismatch inherited from legacy live page copy clone | High | suggested |
