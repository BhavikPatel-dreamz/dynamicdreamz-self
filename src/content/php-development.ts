export const phpDevelopmentContent = {
  hero: {
    title: "PHP Development Company in India",
    description:
      "Dynamic Dreamz is a top PHP Development Company in India. We are specializing in PHP and MySQL. We create custom web solutions to help your business grow. Whether you are a web design company or an individual, our experienced PHP/MySQL developers can help you with your project.",
    ctaLabel: "request a quote",
    ctaHref: "/request-quote",
  },
  brands: {
    title: "Trusted by Leading Brands",
    items: [
      {
        name: "Ranavat Logo",
        src: "/assets/clients/ranavat.svg",
        href: "https://www.ranavat.com/",
        alt: "Ranavat Logo",
        width: 174,
        height: 19,
      },
      {
        name: "prolash_black",
        src: "/assets/clients/prolash.svg",
        href: "https://prolash.com/",
        alt: "prolash_black",
        width: 204,
        height: 22,
      },
      {
        name: "Tropicfeel Logo",
        src: "/assets/clients/tropicfeel.svg",
        href: "https://shop.tropicfeel.com/",
        alt: "Tropicfeel Logo",
        width: 150,
        height: 32,
      },
      {
        name: "perfect_locks_color_logo",
        src: "/assets/clients/perfect-locks.svg",
        href: "https://www.perfectlocks.com/",
        alt: "perfect_locks_color_logo",
        width: 175,
        height: 32,
      },
      {
        name: "Bombay Shirt Company Logo",
        src: "/assets/clients/bombay-shirt-company.svg",
        href: "https://www.bombayshirts.com/",
        alt: "Bombay Shirt Company Logo",
        width: 204,
        height: 26,
      },
      {
        name: "kayfi-colored",
        src: "/assets/clients/kayfi.svg",
        href: "https://kayfi.com/",
        alt: "kayfi-colored",
        width: 90,
        height: 49,
      },
      {
        name: "simdirect_logo_color",
        src: "/assets/clients/simsdirect.svg",
        href: "https://simsdirect.com.au/",
        alt: "simdirect_logo_color",
        width: 143,
        height: 49,
      },
      {
        name: "Kvaser Logo",
        src: "/assets/clients/kvaser.svg",
        href: "https://www.kvaser.com/",
        alt: "Kvaser Logo",
        width: 135,
        height: 25,
      },
      {
        name: "nekter-colored",
        src: "/assets/clients/nelter.svg",
        href: "https://www.nekterjuicebar.com/",
        alt: "nekter-colored",
        width: 66,
        height: 64,
      },
      {
        name: "Circuit City Logo",
        src: "/assets/clients/circuit-city.svg",
        href: "https://circuitcity.com/",
        alt: "Circuit City Logo",
        width: 64,
        height: 64,
      },
    ],
  },
  services: {
    title: "Our PHP Web Development Services",
    subtitle:
      "We create custom eCommerce solutions using Prestashop, a popular online store platform. Our PHP developers build attractive and functional online stores to boost your sales. Hire a PHP Development Company in India to exacl your business.",
    items: [
      {
        title: "Custom Laravel Development",
        description:
          "We build custom web applications using Laravel, a popular PHP framework. Our developers build scalable, high performing, and secure web solutions that are customized to meet your requirements.",
        icon: "/assets/php-development/services/laravel-development.svg",
        iconAlt: "Custom Laravel Development Icon",
      },
      {
        title: "Laravel API Development",
        description:
          "We develop robust APIs with Laravel, allowing your web applications to communicate smoothly with other software. It helps to integrate various services and enhance functionality.",
        icon: "/assets/php-development/services/laravel-api-development.svg",
        iconAlt: "Laravel API Development Icon",
      },
      {
        title: "Laravel Maintenance & Support",
        description:
          "We offer ongoing maintenance and support for your Laravel applications. Our team ensures your web app remains updated, secure, and operates smoothly without any issues.",
        icon: "/assets/php-development/services/laravel-maintenance-support.svg",
        iconAlt: "Maintenance and Support Icon",
      },
      {
        title: "Custom Craft CMS Development",
        description:
          "Our expert team creates custom websites using Craft CMS, a flexible content management system. We build user friendly and feature rich websites to meet your specific requirements.",
        icon: "/assets/php-development/services/craft-cms-development.svg",
        iconAlt: "Custom Craft CMS Development Icon",
      },
      {
        title: "Craft CMS Maintenance",
        description:
          "We provide continuous maintenance services for your Craft CMS website. We ensure optimal performance by having our team update your website and promptly fix bugs.",
        icon: "/assets/php-development/services/craft-cms-maintenance.svg",
        iconAlt: "Craft CMS Maintenance Icon",
      },
      {
        title: "Plugin Development",
        description:
          "We develop custom plugins to extend the functionality of your Craft CMS website. Our plugins can enhance your website's abilities and boost user experience.",
        icon: "/assets/php-development/services/plugin-development.svg",
        iconAlt: "Plugin Development Icon",
      },
      {
        title: "Custom Prestashop Development",
        description:
          "We create custom eCommerce solutions using Prestashop, a popular online store platform. Our developers build attractive and functional online stores to boost your sales.",
        icon: "/assets/php-development/services/prestashop-development.svg",
        iconAlt: "Custom Prestashop Development Icon",
      },
      {
        title: "Module Development",
        description:
          "Our team designs custom modules for Prestashop, adding new features and enhancing your store's functionality. These modules help in providing a better shopping experience for your customers.",
        icon: "/assets/services/magento-development/module-icon.svg",
        iconAlt: "Module Development Icon",
      },
      {
        title: "Prestashop Maintenance & Support",
        description:
          "We offer continuous upkeep and assistance for your Prestashop store. Our staff ensures your store runs smoothly, is safe, and stays updated.",
        icon: "/assets/php-development/services/prestashop-maintenance-support.svg",
        iconAlt: "Prestashop Maintenance & Support Icon",
      },
      {
        title: "API Integration",
        description:
          "We allow your web applications to interact with other programs and services by integrating a variety of APIs into them. It improves functionality and helps streamline processes.",
        icon: "/assets/services/woocommerce-development/api-development.svg",
        iconAlt: "API Integration Icon",
      },
      {
        title: "Custom App Development",
        description:
          "We create custom web apps based on your business requirements. Our team makes sure the apps are effective, safe, and user friendly.",
        icon: "/assets/php-development/services/custom-app-development.svg",
        iconAlt: "Custom App Development Icon",
      },
      {
        title: "Ongoing Support",
        description:
          "We offer continuous support for all our web services. Our team is always ready to help you with any issues or updates you might need.",
        icon: "/assets/services/magento-development/ongoing-support-icon.svg",
        iconAlt: "Ongoing Support Icon",
      },
    ],
  },
  whyDynamicDreamz: {
    heading: "Why Choose Dynamic Dreamz as a PHP Development Company",
    text: "Dynamic Dreamz is a trustworthy PHP development company in India with a proven portfolio of scalable and reliable PHP development solutions. Our skilled development team is an expert in building custom PHP applications that precisely match your company's requirements. Our dedication to quality, timely delivery, and creative solutions guarantee that your PHP project will be managed with professionalism and technical expertise.",
  },
  process: {
    heading: "Our PHP Development Process",
    steps: [
      {
        title: "Discovery and Planning",
        description:
          "We investigate your business requirements, define goals, and design a roadmap based on your needs.",
      },
      {
        title: "Design and Development",
        description:
          "Our team designs user-friendly interfaces and develops robust PHP applications using the latest technologies and best practices.",
      },
      {
        title: "Testing and Deployment",
        description:
          "Strict testing provides bug-free performance and smooth deployment for a hassle-free php application launch.",
      },
      {
        title: "Ongoing Support",
        description:
          "Post-launch, we provide ongoing support, regular updates, and maintenance to keep your PHP application running smoothly.",
      },
    ],
  },
  portfolio: {
    eyebrow: "Portfolio",
    title: "Glimpses of Our PHP/MySQL Development Services",
    subtitle:
      "Dynamic Dreamz isn't just about talk; we're about results. Explore a carefully <br /> curated selection of our successful PHP/MySQL Development projects.",
    ctaLabel: "View our work",
    ctaHref: "/our-work",
    items: [
      {
        name: "Kask",
        category: "PHP",
        href: "https://www.kask.com/en-gb/home",
        image: "/assets/php-development/portfolio/kask.webp",
        imageAlt: "Kask Image",
      },
      {
        name: "No Lawyer",
        category: "PHP",
        href: "https://nolawyers.com.au/",
        image: "/assets/php-development/portfolio/no-lawyer.webp",
        imageAlt: "No Lawyer Image",
      },
      {
        name: "Sims Direct",
        category: "PHP",
        href: "https://simsdirect.com.au/",
        image: "/assets/php-development/portfolio/sims-direct.webp",
        imageAlt: "Sims Direct Image",
      },
      {
        name: "Glass Fit",
        category: "PHP",
        href: "https://glassfit.com.au/",
        image: "/assets/php-development/portfolio/glass-fit.webp",
        imageAlt: "Glass Fit Image",
      },
      {
        name: "Intapol",
        category: "PHP",
        href: "https://www.intapol.com/",
        image: "/assets/php-development/portfolio/intapol.webp",
        imageAlt: "Intapol Image",
      },
      {
        name: "Go Sport Me",
        category: "PHP",
        href: "https://uae.gosportme.com/",
        image: "/assets/php-development/portfolio/go-sport-me.webp",
        imageAlt: "Go Sport Me Image",
      },
    ],
  },
  testimonials: {
    eyebrow: "Client Stories",
    heading: "Don't Just Take Our Word For It",
    description:
      "We have faith in our work, but what truly matters is the outcomes we deliver to our clients. <br> Happy clients make happy stories. Check out how our services empower them to succeed.",
  },
  faqHeading: "Frequently Asked Questions",
  faqs: [
    {
      question: "What is PHP/MySQL development?",
      answer:
        "PHP is a scripting language for creating dynamic web pages, while MySQL is a database management system. Together, they help build and manage robust web applications.",
    },
    {
      question: "Why choose Dynamic Dreamz for PHP/MySQL development?",
      answer:
        "We have a skilled team with years of experience in PHP/MySQL development. We provide customized solutions that meet your business requirements and ensure high performance and security.",
    },
    {
      question: "Can you develop custom web applications?",
      answer:
        "Yes, we are developing custom web applications tailored to your specific needs. Our team ensures the applications are secure, scalable, and user friendly.",
    },
    {
      question: "Why should I use Laravel, and what does it do?",
      answer:
        "The well liked PHP framework Laravel is famous for its effectiveness and simplicity. It facilitates the speedy and safe development of web applications.",
    },
    {
      question: "How long does the process of making a web application take?",
      answer:
        "The complexity of the project determines how long it will take to develop. Once we have reviewed your requirements, we will send you a complete timeline.",
    },
    {
      question: "Is my data secure with you?",
      answer:
        "Absolutely! We follow best practices to ensure your data is secure and protected. Our team prioritizes security in all our development processes.",
    },
    {
      question: "What is the cost of PHP/MySQL development services?",
      answer:
        "The cost depends on the project's requirements and complexity. We offer competitive pricing and will provide a detailed quote after discussing your needs.",
    },
  ],
  ctaBanner: {
    heading: "Want us to help you with your online store?",
    ctaLabel: "request a quote",
    ctaHref: "/request-quote",
  },
};
