...
function DeliverableCard({ item }: { item: (typeof beautyDeliverables.items)[number] }) {
  return (
    <article className="group/card relative isolate h-full min-h-[335px] w-full overflow-visible rounded-[15px] border border-[#efefef] bg-white px-[27px] py-[65px] pr-[27px] pl-[35px] before:pointer-events-none before:absolute before:-inset-[3px] before:-z-2 before:invisible before:rounded-[15px] before:bg-[linear-gradient(to_right,#15c064,#00d1ff)] before:opacity-0 before:transition-opacity before:duration-300 before:content-[''] after:pointer-events-none after:absolute after:inset-0 after:z-0 after:invisible after:rounded-[15px] after:bg-[linear-gradient(179.69deg,rgba(21,192,100,0.1)_-20.68%,rgba(0,209,255,0)_49.11%)] after:opacity-0 after:transition-opacity after:duration-300 after:content-[''] hover:before:visible hover:before:opacity-100 hover:after:visible hover:after:opacity-100 min-[768px]:min-h-[492px] min-[992px]:min-h-[434px] max-[767px]:px-5 max-[767px]:pt-[30px] max-[767px]:pb-[35px]">
...
export function BeautyDeliverablesSection() {
  return (
    <section className="overflow-hidden pb-[3px]" aria-labelledby="beauty-deliverables-title" data-beauty="deliverables">
      <Container className="max-[575px]:px-4">
        <div className="mb-10 flex items-center justify-between -mx-[15px] max-[991px]:mx-0 max-[991px]:flex-col max-[991px]:text-center">
          <div className="w-[calc(35%-15px)] px-[15px] max-[1199px]:w-[calc(40%-15px)] max-[991px]:w-full max-[991px]:p-0">
            <h2
              ...
              id="beauty-deliverables-title"
            >
              {beautyDeliverables.title}
            </h2>
          </div>
          <div className="w-[calc(61.5%-15px)] px-[15px] max-[1199px]:w-[calc(60%-15px)] max-[991px]:w-full max-[991px]:p-0">
            <p className="m-0 text-lg leading-[34.2px] font-medium text-muted max-[991px]:text-base max-[991px]:leading-[30.4px]">
              {beautyDeliverables.description}
            </p>
          </div>
        </div>
      </Container>
      ...
      >
        <div className="flex items-stretch gap-2.5 pl-[25px] min-[768px]:gap-4 min-[1200px]:pl-0">
          {beautyDeliverables.items.map((item) => (
            <div
