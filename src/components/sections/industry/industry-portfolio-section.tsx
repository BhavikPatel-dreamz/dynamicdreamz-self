...
export function IndustryPortfolioSection({ content }: IndustryPortfolioSectionProps) {
  ...
  return (
    <section
      className="pt-20 pb-[85px] max-[992px]:pt-[50px] max-[992px]:pb-[60.4px]"
    ...
    >
      <Container className="max-[575px]:px-4">
        <div className="text-center">
          <h2
            className="mb-2.5 text-[35px] leading-[48.475px] font-bold tracking-[-0.7px] text-ink max-[992px]:text-[30px] max-[992px]:leading-10 max-[767px]:text-2xl max-[767px]:leading-[33.24px] max-[767px]:tracking-[-0.48px]"
        ...
        <div className="mt-[60px] flex flex-wrap gap-x-[15px] gap-y-[60px] max-[992px]:mt-[50px] max-[992px]:gap-y-[30px]">
          {portfolio.items.map((item) => (
            <div
              className="w-[calc(33.33%-10px)] max-[992px]:w-[calc(50%-10px)] max-[767px]:w-full"
              key={item.name}
            >
              <PortfolioProjectCard
                category={item.category}
                categoryClassName="mt-[23px] tracking-[1.12px] max-[992px]:mt-[36.8px]"
                href={item.href}
                image={item.image}
