            ...
            {steps.map((step, index) => (
              <article
                className={cn(
                  ...
                  desktopStepClasses[index] ?? "w-1/4 px-[15px]",
                )}
                key={step.title}
              >
                <div className="relative mb-12 -translate-y-4">
                  <span className="absolute -top-[60px] left-1/2 -translate-x-1/2 bg-[linear-gradient(97deg,#15c064_28.5%,#00d1ff_91.82%)] bg-clip-text font-sans text-[50px] font-bold leading-none text-transparent opacity-60">
                  ...
                  <div className="relative mx-auto flex size-[60px] items-center justify-center rounded-[9.44px] bg-white shadow-[0_4.72px_19.588px_0_rgba(0,0,0,0.05)]">
                    <Image
                      ...
                      className={cn(
                        "object-contain",
                        desktopIconSizes[index] ?? "size-[34px]",
                      )}
                      height={36}
                      src={step.icon}
                      width={36}
                ...
                <h3 className="mb-4 font-montreal-medium text-[18px] font-medium leading-6 tracking-[0.54px] text-ink max-[1199px]:text-base">
                  {step.title}
                </h3>
                <p className="font-sans text-sm font-normal leading-[24.22px] text-[#535353]">
                  {step.description}
                </p>
              </article>
            ))}
